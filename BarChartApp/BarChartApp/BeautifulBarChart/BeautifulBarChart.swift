...
//    private let scrollView: UIScrollView = UIScrollView()
    ...
    private let presenter = BeautifulBarChartPresenter(barWidth: 120, space: -70)
...
//            mainLayer.frame = CGRect(x: 0, y: 0, width: scrollView.contentSize.width, height: scrollView.contentSize.height)
            ...
            contentView.frame.size = CGSize(width: presenter.computeContentWidth(), height: self.frame.size.height)
            

            
            mainLayer.frame = CGRect(x: 0, y: 0, width: contentView.frame.width, height: contentView.frame.height)
    ...
    private func setupView() {
        self.contentView.layer.addSublayer(mainLayer)
